...
class Decoder : public llvm::ModulePass
{
	...
	private:
		...
		Abi* _abi = nullptr;
        std::string _binpath;
        uint64_t codeaddr_begin, codeaddr_end;
        bool codeaddr_boundary_en;
	...
	//
	private:
    inline std::string inst2str(const llvm::Instruction *inst){
        if(inst){
            std::string inststr;
    ...
    inline std::string inst2str(llvm::Instruction *inst){
        if(inst){
            std::string inststr;
            llvm::raw_string_ostream rso(inststr);
            inst->print(rso);
