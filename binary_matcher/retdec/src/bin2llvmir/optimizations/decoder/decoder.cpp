...
bool Decoder::runOnModule(Module& m)
{
    ...
    _binpath = _image->getFileFormat()->getPathToFile();
	return runCatcher();
}
...
void Decoder::getExternalFuncBoundary(){
    string cf = _binpath + ".dcfg";
    ifstream fs;
    fs.open(cf, fstream::in);

    if(codeaddr_boundary_en = fs.good()){
        string word;
        string delim = "##";

        for(; fs >> word; ){
            if(word.find("CODE_BEGIN") == 0){
                string addrstr = word.substr(10 + delim.length());
                stringstream ss;
                ss << addrstr;
                ss >> std::hex >> codeaddr_begin;
            }
            else if(word.find("CODE_END") == 0){
                string addrstr = word.substr(8 + delim.length());
                stringstream ss;
                ss << addrstr;
                ss >> std::hex >> codeaddr_end;
            }
            else if(word.find("ARCH") == 0){
                string mode = word.substr(4 + delim.length());
                _fixedMode = mode;
            }
        }
    }
...
bool Decoder::run()
{
    ...
    // Function boundary getting
    getExternalFuncBoundary();
...
void Decoder::decode()
{
    ...
    if(_fixedMode != "NULL"){
        if(_fixedMode == "ARM_32_ARM") _c2l->modifyBasicMode(CS_MODE_ARM);
        else if(_fixedMode == "ARM_32_THUMB") _c2l->modifyBasicMode(CS_MODE_THUMB);
        else if(_fixedMode == "ARM_64") assert(0);
        else if(_fixedMode == "x86_32") _c2l->modifyBasicMode(CS_MODE_32);
        else if(_fixedMode == "x86_64") _c2l->modifyBasicMode(CS_MODE_64);
        else assert(0);
    }
...
{
    const Address start = jt.getAddress();
    if(codeaddr_boundary_en && (start < codeaddr_begin || start > codeaddr_end)){    // TGKIM's new feature
        _ranges.remove(start, start + 1);                                           // skip out-of-range codes
        return;
    }
    ...
    if (_fixedMode != "NULL" && _c2l->getBasicMode() != jt.getMode())
    {
...
void Decoder::finalizePseudoCalls()
{
	...
	for (auto i = b.begin(), e = b.end(); i != e;)
	{
        ...
        //addr
        Address addr = AsmInstruction::getInstructionAddress(inst);
        if(addr){
            std::stringstream addrss;
            addrss << std::hex << addr;

            LLVMContext& C = inst->getContext();
            MDNode* N = MDNode::get(C, MDString::get(C, addrss.str()));
            inst->setMetadata("addr", N);
        }


        // pseudo call isnt handling
		CallInst* pseudo = dyn_cast<CallInst>(&*i);
		++i;
		if (pseudo == nullptr)
		{
        ...
        Instruction* nextinst = pseudo->getNextNonDebugInstruction();

        if(pseudo->getCalledFunction()->getName().str() == "__indcall" || pseudo->getCalledFunction()->getName().str() == "__pseudo_call"){
            if(CallInst *ci = dyn_cast<CallInst>(nextinst)){
                if(ci->getCalledFunction()->getName().str().find("function_") != string::npos)
                    pseudo->eraseFromParent();// TGKIM // Heuristic based redundant call of direct call w/o removing indirect call
                else{   // TGKIM: Change indcall into user friendly name
                    string indcallname = "__indcall"; //_"  +  ss.str();
                    pseudo->getCalledFunction()->setName(indcallname);

#if 0
                    Address addr = AsmInstruction::getInstructionAddress(pseudo);
                    std::stringstream ss;
                    ss << std::hex << addr;
                    LLVMContext& C = pseudo->getContext();
                    MDNode* N = MDNode::get(C, MDString::get(C, ss.str()));
                    pseudo->setMetadata("addr", N);
#endif
                }
            }
            else{   // TGKIM: Change indcall into user friendly name
        ...
        else{   // not necessary to erase it. But I will erase (target = __pseudo_branch, __pseudo_cond_branch)
