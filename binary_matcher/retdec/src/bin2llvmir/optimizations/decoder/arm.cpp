		...
		bool strict)
{
    ...
    bool print = false;
    if(addr == 0x805ba5c || addr == 0x805ba58){
        print = true;
        std::cout << "> Init Addr: " << std::hex << addr << std::endl;
    }

//        if(addr > 0x805ba58 && addr <0x805ba70 ) return true;


    unsigned count;
	while (count = cs_disasm_iter(ce, &bytes.first, &bytes.second, &addr, _dryCsInsn))
	{
		decodedSz += _dryCsInsn->size;
//    if(addr == 0x805ba5c){
    if(print){
        std::cout << "\t> Addr: " << std::hex << addr << std::dec << " size: " << _dryCsInsn->size << " [" << decodedSz << "/" << bytes.second << "]" << std::endl;
        std::cout << "\t\tcount = " << count << std::endl;
    }

		if (strict && first && !looksLikeArmFunctionStart(_dryCsInsn))
		{
    if(print) std::cout << " return case 1" << std::endl;
			return true;
		}
    ...
    if(print) std::cout << " return case 2" << std::endl;
			return nops;
		}
		...
		{
			if (mode != basicMode) _c2l->modifyBasicMode(basicMode);
    if(print) std::cout << " return case 3" << std::endl;
			return false;
		}
