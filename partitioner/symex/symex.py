...
class SymEx:
    bv_math_funs = ["bvmul", "bvadd", "bvsub", "bvudiv",
                    "bvurem", "bvsdiv", "bvsrem", "bvsmod", "bvneg", "bvabs"]
    debug_info = {}
    ardu_config_types = ["class.AP_ParamT",
...
# name,offset,type
                        self.debug_info[current].append((name.split(
                            ":")[-1].strip(), int(offset.split(":")[-1].strip()), type.split(":")[-1].strip()))

        simple = cxxfilt.demangle(str(z3.simplify(query)))

        if "Concat(0," in simple:
            replacement = simple.split("Concat(0,")[1].split(")")[0]
            simple = simple.replace(
                "Concat(0," + replacement + ")", replacement)
        # or we can just do this?
        # simple = re.sub(r'Concat\(0,([^)]+)\)', r'\1', simple)
                        ...
                        for elem in di:
                            ...
                            if (idx == elem[1]):
                                ...
                                for cfg in self.ardu_config_types:
                                    if (cfg in elem[2]):
                                        base_vec = colorize(
                                            colorize(colorize(base_vec, bcolors.BOLD), bcolors.MAGENTA), bcolors.UNDERLINE)
# TODO check for type
...
# Breakdown If conditions
    ...
    def get_trace(self, id):
        ...
        fun_cov = self.parse_callgrind_file(
            os.environ["P_OUT_DIR"] + id + "/run.istats")
        pp.pprint(fun_cov)
        for fun in fun_cov:
            print(fun + ":")
...
# import pdb; pdb.set_trace()
            while (True):
                ...
                if current not in fun_cov[fun]:
                    print(str(current) + ":"+"  " +
                          linecache.getline(asm_file, current).strip())
    ...
    def generate_summary(self, firmware, function):
        ...
        args = [os.environ["SYMEX"], "--symbolics=obj_list", function, "--threshold",
                str(self.threshold), ("--void_inline" if self.void_inline else ""),  firmware.symex_bc]
        ...
        matching_files = glob.glob(
            os.environ["P_OUT_DIR"] + "klee-last/return_query_*")
        summaries = []
...
# TODO: Hack to consider side effects, this needs to be better
        matching_files = glob.glob(
            os.environ["P_OUT_DIR"] + "klee-last/global_*")
        for file in matching_files:
            with open(file, 'r') as query:
                summary = query.read()
                summaries.append(z3.parse_smt2_string(summary))
